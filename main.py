...
class Calculator(QMainWindow):
    def __init__(self):
        ...
        self.ui.btn_neg.clicked.connect(self.math_operation)
    ...
    def get_tmp_num(self) -> Union[int, float, None]:
        if self.temp.text():
        ...
        if temp:
            try:
                result = self.remove_trailing_zeros(
                    str(operations[self.get_math_sign()](self.get_tmp_num(), self.get_entry_num()))
                )
    ...
    def disable_buttons(self, disable: bool) -> None:
        ...
        color = 'color: #888;' if disable else 'color: #fff;'
    ...
    def adjust_entry_font_size(self) -> None:
        ...
        while self.get_entry_text_width() > self.entry.width() - 15:
            font_size -= 1
            self.entry.setStyleSheet(f'font-size: {str(font_size)}pt; border: none;')
    ...
    def adjust_temp_font_size(self) -> None:
        ...
        while self.get_temp_text_width() > self.temp.width() - 10:
            font_size -= 1
            self.temp.setStyleSheet(f'font-size: {str(font_size)}pt; color: #888;')
